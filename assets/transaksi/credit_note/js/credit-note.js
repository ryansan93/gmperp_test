var cn = {
    ...
    settingUp: function() {
		...
		$('.jenis_cn').select2().select2().on("select2:select", function (e) {
            cn.getTujuan();
        });
    ...
    getTujuan: function() {
        var action = $('#action');
        var jenis_tujuan = $('.jenis_cn').find('option:selected').attr('data-jenis');
    ...
    getLists: function() {
        ...
        if ( err > 0 ) {
            ...
            $.ajax({
                url: 'transaksi/CreditNote/getLists',
                data: { 'params': params },
                type: 'GET',
                dataType: 'HTML',
	...
	loadForm: function(id, edit, href) {
		...
		$.ajax({
            url: 'transaksi/CreditNote/loadForm',
            data: { 'params': params },
            type: 'GET',
            dataType: 'HTML',
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function (result) {
				if ( result ) {
					var params = {
						...
						'no_dok': numeral.unformat($(div).find('.no_dok').val()),
						'supplier': $(div).find('.supplier').select2('val'),
    ...
    delete: function(elm) {
        ...
        bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function (result) {
            if ( result ) {
                ...
                $.ajax({
                    url : 'transaksi/CreditNote/delete',
                    data: { 'params': params },
					type: 'POST',
					dataType: 'JSON',
                    beforeSend : function(){ showLoading() },
