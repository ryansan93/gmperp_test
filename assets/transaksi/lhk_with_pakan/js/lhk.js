...
var lhk = {
	...
	list_riwayat: function(elm) {
		...
		$.ajax({
            url: 'transaksi/LHK_WITH_PAKAN/list_riwayat',
            data: { 'params': params },
            type: 'POST',
            dataType: 'JSON',
	...
	load_form: function(id, edit, href) {
		...
		$.ajax({
            url: 'transaksi/LHK_WITH_PAKAN/load_form',
            data: { 'params': params },
            type: 'POST',
            dataType: 'JSON',
            beforeSend: function(){ showLoading() },
    ...
    get_noreg: function(elm) {
    	...
    	if ( !empty(nomor_mitra) ) {
    		$.ajax({
	            url: 'transaksi/LHK_WITH_PAKAN/get_noreg',
	            data: { 
					'params': nomor_mitra,
					'noreg': noreg,
					'div_id': div_id
	...
	cek_data_pakan: function(elm) {
		...
		if ( !empty($(select_noreg).val()) && !empty($(ipt_tgl).val()) ) {
			var tgl = dateSQL($(div_tgl).data('DateTimePicker').date());
			...
			$.ajax({
				url: 'transaksi/LHK_WITH_PAKAN/data_pakan',
				data: { 'params': params },
				type: 'POST',
				dataType: 'JSON',
				beforeSend: function(){ showLoading('Ambil data pakan . . .') },
				success: function(data){
					if ( data.status == 1 ) {
						$('#modalPakaiPakan').find('.modal-body').html(data.html);
					}

					$('#modalPakaiPakan').modal('show');

					hideLoading();
				}
			});
		} else {
			bootbox.alert('Harap isi noreg dan tanggal terlebih dahulu.');
		}
	}, // end - cek_data_pakan
	...
	uploadSisaPakan: function(elm) {
		...
		ci.compress(files, 480, function(data) {
			...
			$.ajax({
				url: 'transaksi/LHK_WITH_PAKAN/uploadSisaPakan',
				dataType: 'json',
				type: 'post',
				async:false,
				processData: false,
	...
	uploadKematian: function(elm) {
		...
		ci.compress(files, 480, function(data) {
			...
			$.ajax({
				url: 'transaksi/LHK_WITH_PAKAN/uploadKematian',
				dataType: 'json',
				type: 'post',
				async:false,
				processData: false,
	...
	checkboxCheck: function (elm) {
		...
		$.ajax({
			url: 'transaksi/LHK_WITH_PAKAN/createFolderNekropsi',
			data: {
				'id_nekropsi': id,
				'checked': checked,
				'dirStatus': dirStatus
	...
	uploadNekropsi: function(elm) {
		...
		ci.compress(files, 480, function(_data) {
			...
			$.ajax({
				url: 'transaksi/LHK_WITH_PAKAN/uploadNekropsi',
				dataType: 'json',
				type: 'post',
				async:false,
				processData: false,
	...
	preview_file_attachment: function(elm) {
    	...
    	if ( !empty(data_url) && data_url.length > 0 ) {
			$.get('transaksi/LHK_WITH_PAKAN/preview_file_attachment',{
					'data_url': data_url,
					'judul': judul,
					'jenis': jenis
				},function(data){
	...
	get_pakan: function(noreg) {
		$.ajax({
            url: 'transaksi/LHK_WITH_PAKAN/get_pakan',
            data: { 'params': noreg },
            type: 'POST',
            dataType: 'JSON',
            beforeSend: function(){ showLoading() },
	...
	*/
			...
			if ( empty(data_nekropsi) || empty(data_solusi) || err_peralatan > 0 || err_peralatan_cooling_pad_status > 0) {
				...
				bootbox.confirm(ket_confirm, function(result) {
					if ( result ) {
						...
						$.ajax({
							url: 'transaksi/LHK_WITH_PAKAN/save',
							dataType: 'json',
							type: 'post',
							data: {
								'params': data
			...
			if ( empty(data_nekropsi) || empty(data_solusi) || err_peralatan > 0 || err_peralatan_cooling_pad_status > 0) {
				...
				bootbox.confirm(ket_confirm, function(result) {
					if ( result ) {
						...
						$.ajax({
							url: 'transaksi/LHK_WITH_PAKAN/edit',
							dataType: 'json',
							type: 'post',
							data: {
								'params': data
	...
	delete: function(elm) {
		...
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data LHK ?', function(result) {
			if ( result ) {
				$.ajax({
		            url: 'transaksi/LHK_WITH_PAKAN/delete',
		            data: {'params': id},
		            type: 'POST',
	            	dataType: 'JSON',
		            beforeSend: function() {
	...
	deleteFile: function (elm) {
		...
		$.ajax({
			url: 'transaksi/LHK_WITH_PAKAN/deleteFile',
			data: {'url': url},
			type: 'POST',
			dataType: 'JSON',
			beforeSend: function() {},
