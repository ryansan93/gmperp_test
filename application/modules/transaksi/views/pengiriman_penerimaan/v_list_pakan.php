<?php if ( !empty($data_pakan) && count($data_pakan) > 0 ): ?>
	<?php foreach ($data_pakan as $k => $v_data): ?>
		<tr class="search">
			...
			<td>
				<div class="col-sm-6 no-padding text-center cursor-p btn_action">
					<i class="fa fa-file" style="color:white;" title="VIEW" data-id="<?php echo $v_data['id']; ?>" onclick="pp.changeTabActive(this)" data-href="pengiriman"></i>
				</div>
				<div class="col-sm-6 no-padding text-center cursor-p btn_action">
					<i class="fa fa-list" style="color:white;" title="LIST AKTIFITAS" data-id="<?php echo $v_data['id']; ?>" onclick="pp.listActivity(this)"></i>
				</div>
			</td>
		</tr>
	<?php endforeach ?>
